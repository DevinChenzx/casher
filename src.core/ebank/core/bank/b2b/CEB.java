...
public class CEB extends BankExt implements BankService{
	...
	public String sendOrderToBank(BankOrder order) throws ServiceException {
		...
		String plain="transId=EPER~|~merchantId="+this.getCorpid()+"~|~orderId="+
		order.getRandOrderID()+"~|~transAmt="+order.getAmount()+"~|~transDateTime="+order.getPostdate()+
		"~|~currencyType=01~|~customerName=~|~productInfo=~|~customerEMail=~|~merURL="+this.getHttprecurl()+"&NR=SID_"+this.idx+"" +
				"~|~merURL1="+this.getRecurl()+"~|~msgExt=";
