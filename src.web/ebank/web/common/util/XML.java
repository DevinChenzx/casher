...
public class XML {
	...
	private static Map<String, String> parseElements(Map<String, String> map, Element element, String name) throws Exception {
		...
		{
			map.put(name.trim().equals("") ? element.getName().toLowerCase() : name, element.getText());
		}
		else
		{
			for(Element _element : elementList)
			{
				map = parseElements(map, _element, name.trim().equals("") ? null : (name + "." +_element.getName().toLowerCase()));
			}
		}
