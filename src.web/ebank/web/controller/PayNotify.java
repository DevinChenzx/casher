...
public class PayNotify implements Controller{
	...
	private String getAdaptorBankID(HttpServletRequest request)throws ServiceException {
		...
		if(request.getParameter("idx")!=null){
		...
		}else if(request.getParameter("merVAR")!=null && request.getParameter("notifyData") != null){ //工行 version 1.0.0.11
			...
			bankID=String.valueOf(RequestUtil.HtmlEscape(request.getParameter("MerchantPara")));  //招行处理
		...
		}else if(request.getParameter("remark")!=null && request.getParameter("remark").equals("0908")){//汇付
	...
	public ModelAndView handleRequest(HttpServletRequest request,
		...
		try{			
			...
			if((BankCode.BKID_ICBC[0].equals(bankID.substring(0,3))&&!reqs.containsKey("v_md5info"))
					...
					||(BankCode.BKID_BOB[0].equals(bankID))&&!(reqs.containsKey("v_info"))//北京银行，工商银行,广银联
				  ){ 
			...
			GwOrders gw10=orderService.findOrderByTrx(result.getTransnum(),result.getBankcode()); //网银的订单号
			...
			if(gw20==null){
				...
				if("1".equals(gw20.getTrxsts())){ //charge without payment or repeated refresh	
					...
					if("2".equals(reqs.get("r9_BType"))) return null;//易宝支付时，交易类型是2时为服务器点对点通讯
